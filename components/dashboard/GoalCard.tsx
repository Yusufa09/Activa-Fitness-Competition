    ...
    >
      <div className="flex items-start justify-between gap-3">
            ...
            <div className="ml-6 mt-3">
              <div className="flex items-center gap-2 mb-1">
                {Array.from({ length: goal.target_count }).map((_, i) => (
                  <div
                    key={i}
                    className={`flex-1 h-2 rounded-full ${i < goal.progress ? "bg-orange-500" : "bg-slate-200 dark:bg-slate-600"}`}
                  />
                ))}
              </div>
        ...
        <div className="flex flex-col items-end gap-2 flex-shrink-0">
          <span className="text-sm font-bold text-amber-600 dark:text-amber-400">+{goal.points} pts</span>
          {goal.completed ? (
